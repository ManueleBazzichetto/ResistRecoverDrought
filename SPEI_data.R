...
##Extract SPEI data for the regions of the Biodiversity Exploratories, have a look at the trend of SPEI values
...
ALB_map <- ggplot() +
  ...
  scale_fill_gradientn(colours = terrain.colors(20), name = "Altitude m asl") +
...
ggsave(plot = ALB_map, filename = "~/Documents/ClimateExtremes/CleanCode/PNG_figs/ALB_map.png", device = "png", width = 9, height = 5, bg = "white")
...
ggsave(plot = HAI_map, filename = "~/Documents/ClimateExtremes/CleanCode/PNG_figs/HAI_map.png", device = "png", width = 9, height = 9, bg = "white")
...
ggsave(plot = SCH_map, filename = "~/Documents/ClimateExtremes/CleanCode/PNG_figs/SCH_map.png", device = "png", width = 9, height = 9, bg = "white")
...
#palette for colours to associate with drought intensity - palette changed to 'Roma' on nov 20th (2023)
Drought_pal <- setNames(object = hcl.colors(n = 5, palette = "Roma"), nm = c('ExtremeDry', 'ModerateDry', 'Normal', 'ModerateWet', 'ExtremeWet'))

#color for 'Normal' is too light. Replace with last color of hcl.colors(n = 5, palette = "Tofino")
Drought_pal["Normal"] <- "#C2EFB4"
...
MainSPEI_plot <- ggplot(SPEI_nc.may.plot[SPEI_nc.may.plot$Type %in% c("SPEI3", "SPEI12"), ], aes(x = Year, y = Val, group = Type)) +
  ...
  scale_linetype_manual(values = c("SPEI3" = 3, "SPEI12" = 1), labels = c('SPEI3' = "SPEI-3", 'SPEI12' = "SPEI-12"),
                        name = "SPEI time-scale") +
  geom_point(aes(pch = Type), cex = 3) +
  scale_shape_manual(values = c("SPEI3" = 16, "SPEI12" = 18), labels = c('SPEI3' = "SPEI-3", 'SPEI12' = "SPEI-12"),
                     name = "SPEI time-scale") +
  facet_wrap(~ Explo, labeller = labeller(Explo = c(A = "South-West", H = "Center", S = "North-East"))) +
  ylab("SPEI May") + xlab(NULL) +
  theme_pubr() + theme(axis.text.x.bottom = element_text(angle = 45, hjust = 1, vjust = 1, size = 11),
                       legend.position = "bottom", strip.background = element_blank(), strip.text = element_blank(),
                       axis.title.y = element_text(size = 14), legend.title = element_text(size = 12),
                       legend.text = element_text(size = 12))


#figure for manuscript combining trend in raw biomass and SPEI-3 and -12 (created on 29th Nov 2023)

BiomassSPEI_trend_plot <- ggarrange(Biomass_plot_trend, MainSPEI_plot, nrow = 2, labels = "auto")

ggsave(plot = BiomassSPEI_trend_plot, filename = "~/Documents/ClimateExtremes/CleanCode/New_figures/Biomass_SPEI_trend.jpeg", device = "jpeg", units = "cm", dpi = 300, width = 22, height = 25, bg = "white")
